...
function Post({ id, userImg, username, caption, img }) {
  ...
  return (
    <div className='bg-white my-7 border rounded-md '  >
        ...
        {comments.length > 0 && (
            <div className="ml-10 h-20 scrollbar-thumb-black overflow-y-scroll   "  >
                {comments.map((comment) => (
                  <div className='flex space-x-3 mt-2 mb-2 items-centre'   key={comment.id}   >
                    <img  className='h-7 rounded-full'   src={comment.data().userImg} />
        ...
        {session && (
            <form className="flex items-centre justify-between p-4 bg-white-200 cursor-pointer" >
                ...
                <input type="text" value={comment} placeholder="Add a comment..." 
                  ...
                  className="flex-1 border-none outline-none space-x-3" 
                />
                <button className="" type="submit" onClick={sendComment} >
                    post
                </button>
            </form>
        )}
